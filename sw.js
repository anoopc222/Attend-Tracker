...
const CACHE_VER = 'at-pro-v3';
const SHELL_CACHE = `${CACHE_VER}-shell`;
const FONT_CACHE  = `${CACHE_VER}-fonts`;

/* Files that must be available offline — static assets only (not index.html) */
const SHELL_ASSETS = [
  './manifest.json',
  './icon-192.png',
  './icon-512.png',
...
self.addEventListener('fetch', event => {
  ...
  // 2. Shell static assets (icons, manifest) — cache first
  if (SHELL_ASSETS.some(a => url.pathname.endsWith(a.replace('./', '/')))) {
    event.respondWith(cacheFirst(request, SHELL_CACHE));
    return;
  }

  // 3. index.html / navigation — NETWORK FIRST so updates always load
